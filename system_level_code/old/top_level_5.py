...
import system_specs_8
import sys
import practice_plots_4
...
def main():
     ...
     for (array_size_idx, array_size) in enumerate(array_size_options):
          ...
          SS_in_out_wanted[array_size_idx] = pd.DataFrame(SS_inputs + SS_outputs, index = SS_in_out_names, columns = ["0"])
          ...
          for column_name in SS_in_out_saved:
               if (SS_in_out_wanted[array_size_idx].loc[SS_inputs_names, "0"].equals(SS_in_out_saved.loc[SS_inputs_names, column_name])):
                    print("found matching SS inputs at column", column_name, ",will now load corresponding outputs")
                    need_run_SS = 0
                    SS_in_out_wanted[array_size_idx].at[SS_outputs_names, "0"] = SS_in_out_saved.loc[SS_outputs_names, column_name]
                    break
     ...
     for array_size_idx in range(len(SS_in_out_wanted)):
          SS_in_out_wanted[array_size_idx] = SS_in_out_wanted[array_size_idx].rename(columns={"0": "SR: " + str(base_SR)})
          for SR in symbol_rate_options[1:]:
               SS_in_out_wanted[array_size_idx].insert(SS_in_out_wanted[array_size_idx].shape[1], "SR: " + str(SR), SS_in_out_wanted[array_size_idx].iloc[:, 0])
               
     
     for (array_size_idx, array_size_chip_specs_df) in enumerate(chip_specs):
          chip_specs[array_size_idx] = pd.DataFrame(index = chip_specs_names)
          col_name = "SR: " + str(base_SR)
          SS_inputs  = SS_in_out_wanted[array_size_idx].loc[SS_inputs_names,  col_name]
          SS_outputs = SS_in_out_wanted[array_size_idx].loc[SS_outputs_names, col_name]
          for SR in symbol_rate_options:
               col_name = "SR: " + str(SR)
               chip_specs_single = system_specs_8.run_power_area_model(SS_outputs, SS_inputs, SR * ghz)
               chip_specs[array_size_idx].insert(chip_specs[array_size_idx].shape[1], col_name, chip_specs_single)
     ...
     fig = practice_plots_4.make_stacked_bar_plot(chip_specs, params_interest, params_total_quantities, params_other_names, repeat_tick_labels, array_configs, "mW")
